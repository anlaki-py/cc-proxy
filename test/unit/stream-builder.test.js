...
function parseSSE(s) {
  ...
  for (const block of s.split(/\n\n/)) {
    ...
    if (event && data) {
      try { events.push({ event, data: JSON.parse(data) }); } catch (e) { /* skip */ }
    }
  }
  return events;
}
...
test('StreamBuilder: openThinking then text auto-closes the thinking block', () => {
  ...
  const textStartIdx = events.findIndex(e => e.event === 'content_block_start' && e.data.content_block.type === 'text');
...
test('StreamBuilder: text after openToolCall does not crash', () => {
  ...
  assert.doesNotThrow(() => { out += b.text('nope'); });
